...
void rotate(int dim, pixel *src, pixel *dst) 
{
	...
	int dim_1 = dim - 1;
	pixel *src_1 = src;
	for (i = dim * (dim - 1); i != dim_1; i = i + dim) {
		for (j = dim; j != 0; j--, i = i - dim)
			*(dst + i) = *src_1;
		src_1++;
	}
	*(dst + i) = *src_1;
...
void naive_smooth(int dim, pixel *src, pixel *dst) 
{
	...
	for (j = 1; j < dim - 1; j++) {
		//i == 0;
		dst[j].red = (src[].red + src[(tmp + 1)].red + src[(tmp + 2)].red + src[(tmp + dim)].red + src[(tmp + dim + 1)].red + src[(tmp + dim + 2)].red) / 6;
